...
class Model_permohonan extends CI_Model
{
   public function getallpermohonan()
   {
      if($this->session->userdata('level')=='pemohon'){
         ...
         $query = "SELECT * from tb_permohonan where kode != '3' and status != 'surat terbit' and id_user='$id'";
         return $this->db->query($query)->result_array();
      }else{
          $query = "SELECT * from tb_permohonan where kode != '3' and status != 'surat terbit'";
         return $this->db->query($query)->result_array();
      }
   }
   ...
   {
      $this->db->delete('tb_permohonan', ['id_mohon' => $id]);
   }
